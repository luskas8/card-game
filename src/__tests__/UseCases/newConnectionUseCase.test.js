...
describe("newConnectionUseCase", () => {
    ...
    it("should not be able to connect when game is full", async () => {
        const response = await newConnectionUseCase("socketID 1", { name: "Jailson Mendes 1" }).then(async () => {
            return await newConnectionUseCase("socketID 2", { name: "Jailson Mendes 2" }).then(async () => {
                return await newConnectionUseCase("socketID 3", { name: "Jailson Mendes 3" }).then(async () => {
                    return await newConnectionUseCase("socketID 4", { name: "Jailson Mendes 4" }).then(async () => {
                        return await newConnectionUseCase("socketID 5", { name: "Jailson Mendes 5" }).then(async () => {
                            return await newConnectionUseCase("socketID 6", { name: "Jailson Mendes 6" }).then(async () => {
                                return await newConnectionUseCase(socketID, { name })
                            })
                        })
                    })
                })
